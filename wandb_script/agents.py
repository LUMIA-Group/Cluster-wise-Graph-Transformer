...
def agent(entity, project, q, sweep_id, gpu_index, agent_package, code_fullname, save_model,):
    print('Agent started with GPU [%s].'%(gpu_index))
    try:
        wandb.agent(sweep_id, function=lambda:runner(sweep_id, gpu_index, code_fullname, save_model,), entity=entity, project=project, count=agent_package)
    except:
        time.sleep(180)
...
if __name__ == '__main__':
    ...
    while True:
        if num_now<num_space:
            gpu_index = q.get()
            if args.mode=='parallel':
                p = Process(target=agent, args=(args.entity, args.project, q, args.sweep_id, gpu_index, agent_package, code_fullname, save_model))
                p.start()
            elif args.mode=='one-by-one':
                agent(args.entity, args.project, q, args.sweep_id, gpu_index, agent_package, code_fullname, save_model)
            num_now += 1
            print(num_now)
        else:
            break
